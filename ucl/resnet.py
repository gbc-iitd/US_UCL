...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    for param_model in kwargs.items():
        name_param_model = param_model[0]
        if name_param_model != 'num_classes':
            arbit_keyword_arg[name_param_model]=kwargs[name_param_model]
    
    model = ResNet(block, layers, **arbit_keyword_arg)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
    model.fc = nn.Linear(model.fc.weight.shape[1],kwargs['num_classes'])
    return model
