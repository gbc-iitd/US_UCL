...
class R2V2Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, data_split='train', return_all_video_frames=False,
        ...
        self.return_neg_frame = return_neg_frame  ## False
    ...
    def __getitem__(self, index):
        ...
        key_q = index
        #key_q = 1e6 * index + ind[0]
        #"""
        #image.save("Original_withoutaug_"+str(index)+".jpg")
        #"""
        ...
        if self.transform is not None:
            ...
            img_temp = np.swapaxes(img_temp,0,2)
            img_temp = np.swapaxes(img_temp,0,1)
            img_temp *= 255
            cv2.imwrite("Original_withaug_"+str(index)+".jpg",img_temp)
            ...
            img_temp = np.swapaxes(img_temp,0,1)
            img_temp *= 255
            cv2.imwrite("Original_withaug2_"+str(index)+".jpg",img_temp)
            """
        ...
        if self.num_of_sampled_frames != -1:
            ...
            key_k = index
            #key_k = 1e6 * index + sampled_frame_ids[0]
            #"""
            #for video_frame_temp in video_frames:
            #    video_frame_temp.save("PosPair_withoutaug_"+str(index)+".jpg")
            #"""
            ...
            for video_frame_temp in video_frames:
                ...
                video_frame_temp = video_frame_temp.detach().cpu().numpy()
                video_frame_temp = np.swapaxes(video_frame_temp,0,2)
                video_frame_temp = np.swapaxes(video_frame_temp,0,1)
                video_frame_temp *= 255
                cv2.imwrite("PosPair_withaug_"+str(index)+".jpg",video_frame_temp)
            """
            ...
            metadata_q = torch.tensor([key_q],dtype=torch.int64)
            metadata_k = torch.tensor([key_k],dtype=torch.int64)
            #metadata = torch.from_numpy(metadata)

            return sample, target, index, video_frames,metadata_q,metadata_k
